...
def _select_first_available(series, candidates, default=None):
    """Return the first non-null value for the given candidate keys."""
...
def _prepare_statements(income_stmt, balance_sheet, cash_flow):
    ...
    # Columns come back as Periods/Datetimes. Sort latest->earliest for consistency.
    ...
    if len(income_cols) < 2 or len(balance_cols) < 2 or len(cash_cols) < 2:
        raise ValueError("Not enough historical data (need at least two years).")
...
def extract_data(income_stmt, balance_sheet, cash_flow):
    ...
    fields = {
        'revenue': None, 'net_income': None, 'gross_profit': None, 'receivables': None,
        'current_assets': None, 'total_assets': None, 'ppe': None, 'depreciation': None,
        'sga_expenses': None, 'current_liabilities': None, 'long_term_debt': None,
        'nonoperating_income': None, 'operating_cash_flow': None,
        'revenue_previous': None, 'net_income_previous': None, 'gross_profit_previous': None,
        'receivables_previous': None, 'current_assets_previous': None, 'total_assets_previous': None,
        'ppe_previous': None, 'depreciation_previous': None, 'sga_expenses_previous': None,
        'current_liabilities_previous': None, 'long_term_debt_previous': None,
        'nonoperating_income_previous': None, 'operating_cash_flow_previous': None
    }

    # Current year
    fields['revenue'] = _select_first_available(
        inc_current,
        ['Total Revenue', 'Revenue'],
    )
    fields['net_income'] = _select_first_available(
        inc_current,
        ['Net Income', 'NetIncome'],
    )
    fields['gross_profit'] = _select_first_available(
        inc_current,
    ...
    fields['receivables'] = _select_first_available(
        bal_current,
        ['Accounts Receivable', 'Receivables'],
    )
    fields['current_assets'] = _select_first_available(
        bal_current,
        ['Total Current Assets', 'Current Assets'],
    )
    fields['total_assets'] = _select_first_available(
        bal_current,
        ['Total Assets'],
    )
    ...
    # Previous year
    fields['revenue_previous'] = _select_first_available(
        inc_previous,
        ['Total Revenue', 'Revenue'],
    )
    fields['net_income_previous'] = _select_first_available(
        inc_previous,
        ['Net Income', 'NetIncome'],
    )
    fields['gross_profit_previous'] = _select_first_available(
        inc_previous,
        ['Gross Profit', 'GrossProfit'],
    )
    ...
    fields['total_assets_previous'] = _select_first_available(
        bal_previous,
        ['Total Assets'],
    )
...
app_ui = ui.page_fluid(
    ui.panel_title("Beneish M-Score Calculator"),
    ui.row(
        ui.column(
            4,
            ui.input_text(id="ticker_input", label="Enter a stock ticker (e.g., AAPL)"),
            ui.input_action_button(id="submit", label="Calculate M-Score", class_="btn-success"),
        ),
        ui.column(
            8,
...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def result():
        ...
        try:
            ...
            return f"Unable to retrieve financial data for {ticker.upper()}: {exc}"
        ...
        if m_score is not None:
            ...
            if m_score > -2.22:
                response += "Higher risk of earnings manipulation."
            else:
                response += "Lower risk of earnings manipulation."
        else:
            response += "\nUnable to calculate M-Score (missing data for TATA)."
