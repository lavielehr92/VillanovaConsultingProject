...
with st.sidebar:
    ...
    max_distance = st.slider("Max Distance from CCA (km)", 
                            ...
                            value=20.0, step=0.5, key="distance_slider",
    ...
    edi_range = st.slider("Educational Desert Index Range", 
                         ...
                         step=1.0, key="edi_range")
    
    # Income Filter
    income_range = st.slider("Household Income Range", 
                            min_value=int(demographics['income'].min()), 
                            max_value=int(demographics['income'].max()), 
                            value=(int(demographics['income'].min()), int(demographics['income'].max())),
                            step=1000,
                            format="$%d", key="income_range")
    
    # School Type Filter
    school_types = st.multiselect("School Types", 
                                 schools['type'].unique(), 
                                 default=schools['type'].unique(), key="school_types")
    ...
    poverty_range = st.slider("Poverty Rate Range (%)", 
                             ...
                             step=0.5, key="poverty_range")
    
    # Target Family Filters
    st.subheader("Target Family Criteria")
    min_first_gen = st.slider("Min % First Generation", 0, 100, 0, key="first_gen")
    min_christian = st.slider("Min % Christian", 0, 100, 0, key="christian")
    ...
    income_preset = st.selectbox("Income Target Preset", 
                                ["Custom Range", "Premium Tier ($150K+)", "High Income ($100K+)", 
                                 "Upper Middle ($75K+)", "Middle Class ($50K+)", "All Incomes"], key="income_preset")
    ...
    show_marketing_priority = st.checkbox("Highlight Marketing Priority Areas", value=True, key="marketing_toggle")
...
if 'distance_to_cca' in map_data.columns:
    ...
    st.sidebar.info(f"📍 Showing {len(map_data_filtered)} of {len(map_data)} ZIP codes within {max_distance}km")
else:
    map_data_filtered = map_data
    st.sidebar.info(f"📍 Showing all {len(map_data)} ZIP codes")

# Check if we have any data to plot
if len(map_data_filtered) == 0:
    st.error(f"No ZIP codes found within {max_distance}km of CCA campuses. Try increasing the distance filter.")
    st.stop()

filtered_schools = schools[schools['type'].isin(school_types)]
# Exclude CCA campuses from school plotting (they get yellow stars separately)
filtered_schools = filtered_schools[~filtered_schools['school_name'].str.contains('Cornerstone Christian Academy', na=False)]

# Create scatter map (removed boundaries due to data matching issues)
# Prepare hover data conditionally
scatter_hover = hover_data.copy()
if 'distance_to_cca' in map_data_filtered.columns:
    scatter_hover.append('distance_to_cca')
...
# Update marker size only for scatter traces (not choropleth or CCA campuses)
for trace in fig_edi_map.data:
    if hasattr(trace, 'marker') and hasattr(trace.marker, 'sizemin'):
        # Don't modify CCA campus markers (they should stay yellow stars)
        if trace.name not in ['CCA Main Campus (58th St)', 'CCA Baltimore Ave Campus']:
...
fig_edi_map.update_layout(
    ...
    legend=dict(
        ...
        y=0.98,
        xanchor="left",
        x=0.02,  # Position legend inside the plot area on the left
        bgcolor="rgba(255,255,255,0.9)",
        bordercolor="rgba(0,0,0,0.3)",
        borderwidth=1,
        font=dict(size=10)  # Smaller font to reduce size
    ),
    margin=dict(l=0, r=0, t=50, b=0)  # Remove right margin since legend is now inside
)

st.plotly_chart(fig_edi_map, key="main_map", width='stretch')
